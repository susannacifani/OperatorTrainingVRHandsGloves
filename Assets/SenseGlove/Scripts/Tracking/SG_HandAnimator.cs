...
namespace SG
{
    ...
    public class SG_HandAnimator : SG_HandComponent
    {
        ...
        public Quaternion[][] sendJoints; // Riferimento a SendJoints
        ...
        public static void SetHandToPose(SG_HandModelInfo handModelInfo, SG_HandPose pose, bool fingersOnly = false)
        {
            if (pose != null)
            {
                ...
                //Quaternion uniformAngle = Quaternion.Euler(0, 0, -90); // Rotazione uniforme di 90° con calcolo completo
                //Quaternion uniformAngle = Quaternion.Euler(0, -90, -90); // Rotazione uniforme di 90° X
                //Quaternion uniformAngle = Quaternion.Euler(90, 270, 0); // Rotazione uniforme di 90° Y
                //Debug.Log($"Quaternion: {uniformAngle}");
                //Debug.Log($"Euler Angles: {uniformAngle.eulerAngles}");

                //fingerJoints[0][0].rotation = SendJoints.kanevJoints[0][0];
                ////Debug.Log($"Primo elemento di kanevJoints: {SendJoints.kanevJoints[0][0]}");
                //fingerJoints[0][1].rotation = SendJoints.kanevJoints[0][1];
                //fingerJoints[0][2].rotation = SendJoints.kanevJoints[0][2];
                //fingerJoints[0][3].rotation = SendJoints.kanevJoints[0][3];


                for (int f = 0; f < fingerJoints.Length; f++)
                {
                    for (int j = 0; j < fingerJoints[f].Length; j++) //for (int j = 0; j < fingerJoints[f].Length; j++)
                    {

                        fingerJoints[f][j].transform.localRotation = SendJoints.kanevJoints[f][j];
                        //fingerJoints[0][j].rotation = SendJoints.kanevJoints[0][j];
                ...
                //                    //fingerJoints[f][j].rotation = uniformAngle;
                //                }
                //                //Debug.Log($"Angle = {angles[f][j]}");
                //            }
                //        }
                //
                //
        ...
        protected virtual void Start()
        {
            ...
            sendJoints = SendJoints.GetKanevJoints();
        }
        ...
        protected virtual void Update()
        {
            ...
            //Debug.Log($"Primo elemento di kanevJoints: {sendJoints}");
