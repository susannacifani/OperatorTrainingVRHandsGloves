...
namespace Impostors.Editor
{
    ...
    internal class ImpostorLODGroupEditor : UnityEditor.Editor
    {
        ...
        private bool _foldoutInfoAboutDisabledAtStartupRenderers = true;
        private bool _foldoutWarningAboutAlwaysVisibleRenderers = true;
        ...
        private void DrawHelpBoxAboutUnassignedRenderers()
        {
            ...
            if (presentOnlyInImpostor.Count != 0)
            {
                string infoMessage =
                    ...
                    $"Such renderers will be automatically disabled at startup.\n\n" +
                    $"This is totally okay if it's intended.";

                EditorGUI.indentLevel++;
                _foldoutInfoAboutDisabledAtStartupRenderers =
                    EditorGUILayout.Foldout(_foldoutInfoAboutDisabledAtStartupRenderers, $"Renderers that will be disabled at startup ({presentOnlyInImpostor.Count})", true);

                if (_foldoutInfoAboutDisabledAtStartupRenderers)
                {
                    EditorGUILayout.HelpBox(infoMessage, MessageType.Info);
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUI.indentLevel++;
                    foreach (var renderer in presentOnlyInImpostor)
                    {
            ...
            if (freeRenderers.Count != 0)
            {
                string warningMessage =
                    ...
                    $"Which means that visibility of these renderers is NOT controlled by anything.\n\n" +
                    $"This is totally okay if it's intended.";

                EditorGUI.indentLevel++;
                _foldoutWarningAboutAlwaysVisibleRenderers =
                    EditorGUILayout.Foldout(_foldoutWarningAboutAlwaysVisibleRenderers, $"Renderers that will be always visible ({freeRenderers.Count})", true);

                if (_foldoutWarningAboutAlwaysVisibleRenderers)
                {
                    EditorGUILayout.HelpBox(warningMessage, MessageType.Warning);
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUI.indentLevel++;
                    foreach (var renderer in freeRenderers)
                    {
                        EditorGUILayout.ObjectField(renderer, typeof(Renderer), true);
                    }
        ...
        private void DrawSetPassCallsDetails()
        {
            var renderers = m_ImpostorLODGroup.LODs[activeLOD].renderers;
            var materials = new List<Material>();
            int setPassCount = 0;
            foreach (var r in renderers)
            {
                r.GetSharedMaterials(materials);
                setPassCount += materials.Count;
            }
            ...
            if (setPassCount < 10)
            {
                EditorGUILayout.HelpBox($"Rendering of this LOD will cost {setPassCount} SetPass calls.", MessageType.Warning);
                return;
            }

            EditorGUILayout.HelpBox($"Rendering of this LOD will cost {setPassCount} SetPass calls.\n" +
                                    $"Consider excluding some renderers if they are not important.\n" +
                                    $"Alternatively, you could combine some objects into one mesh (not Static Bathing).", MessageType.Error);
        }
